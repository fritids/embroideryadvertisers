...
function eStore_get_buy_now_submit_input($button_img_url,$button_text='')
{
    ...
    if (!empty($button_img_url)){
        $replacement .= '<input type="image" src="'.$button_img_url.'" class="eStore_button wppg-eStore-buy-button-submit" alt="'.WP_ESTORE_BUY_NOW.'" />';
    }
    else{
    	if(empty($button_text)){$button_text = WP_ESTORE_BUY_NOW;}
		$replacement .= '<input type="submit" value="'.__($button_text).'" class="eStore_button wppg-eStore-buy-button-submit" />';
    }
    return $replacement;
}
...
function eStore_get_gateway_specific_buy_now_button_code($id,$gateway='paypal',$button_text='',$line_break=true,$nggImage='',$buttonImage='')
{
    ...
    if($ret_product->show_qty=='1'){
		$replacement .= '<div class="wppg-eStore-buy-button-qty">'.ESTORE_QUANTITY.': <input type="text" name="add_qty" size="2" value="1" /></div>';			
    }
    else{
