...
function get_button_code_for_element($ret_product,$line_break=true,$nggImage='',$buttonImage='')
{
	...
	if($ret_product)
	{
	    ...
	    {
			$replacement .= ESTORE_QUANTITY.': <input type="text" name="add_qty" size="2" value="1" />&nbsp;';
			if ($line_break) $replacement .= '<br />';
	    }
	    else
	    {
...
function get_button_code_fancy2_for_element($ret_product,$line_break=true)
{
	...
	{	
	    ...
	    $replacement .= '<input type="hidden" name="thumbnail_url" value="'.$ret_product->thumbnail_url.'" />';
		$replacement .= '<input type="hidden" name="product" value="'.htmlspecialchars($ret_product->name).'" /><input type="hidden" name="price" value="'.$ret_product->price.'" />';
...
function print_eStore_buy_now_button($id,$button='',$nggImage='')
{
    ...
    if($ret_product->show_qty=='1')
    {
		$output .= ESTORE_QUANTITY.': <input type="text" name="quantity" size="2" value="1" />&nbsp;';
		if ($line_break) $output .= '<br />';
    }		
	    
    if(!empty($nggImage->alttext))
    {
    	$output .= '<input type="hidden" name="product_name_tmp1" value="'.htmlspecialchars($nggImage->alttext).'" /><input type="hidden" name="price_tmp1" value="'.$ret_product->price.'" />';
    }
    else
    {
    	$output .= '<input type="hidden" name="product_name_tmp1" value="'.htmlspecialchars($ret_product->name).'" /><input type="hidden" name="price_tmp1" value="'.$ret_product->price.'" />';
    }
...
function eStore_donate_button_code($args){
	...
	else{$notify = WP_ESTORE_URL.'/paypal.php';}
...
function show_product_fancy_style($id,$button_type=1,$show_price=1,$restriction='')
{
	global $wpdb;
	$products_table_name = WP_ESTORE_PRODUCTS_TABLE_NAME;
	$ret_product = $wpdb->get_row("SELECT * FROM $products_table_name WHERE id = '$id'", OBJECT);
	...
	if($show_price==1){        
	    if(!empty($ret_product->old_price)){
	    	$output .= '<div class="eStore_oldprice"><strong>'.ESTORE_OLD_PRICE.': </strong>'.print_tax_inclusive_payment_currency_if_enabled($ret_product->old_price, WP_ESTORE_CURRENCY_SYMBOL,'',$ret_product).'</div>';
	    }
    ...
    $output .= eStore_show_button_based_on_condition($id,$ret_product,$button_type,$restriction);
    $output .= '</div></div>';
...
function show_product_fancy_style2($id,$button_type=1,$show_price=1,$restriction='')
{
	...
	$output .= eStore_show_button_based_on_condition($id,$ret_product,$button_type,$restriction,'2');
    $output .= '</div></div>';
...
{
	if (isset($_POST['eStore_download_now_button']) && eStore_multi_submit_check())
	{
		//sanitize data
	    ...
	    eStore_redirect_to_url($full_encrypted_url,'0','0');	    
	}
...
function eStore_show_button_based_on_condition($id,$ret_product,$button_type,$restriction,$style='')
{
	$output = "";
	$show_button = false;
...
function eStore_show_product_details($id,$info)
{
	...
	if(isset($product_details->$info)){
		...
		if(!empty($product_details->tax)){$tax_rate = $product_details->tax;}
    	else{    		
