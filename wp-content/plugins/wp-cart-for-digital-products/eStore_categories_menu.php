...
function wp_eStore_manage_categories_menu()
{
	...
	if (isset($_POST['Submit']))
	{
		...
		$cat_name = $wpdb->escape($_POST['cat_name']);
		...
		$cat_desc = $wpdb->escape($tmpdescription);
		$cat_parent=0;//if ($cat_parent=="") $cat_parent=0;//$wpdb->escape($_POST['cat_parent']);
		$cat_url = $wpdb->escape($_POST['cat_url']);	
		$cat_image = $wpdb->escape($_POST['cat_image']);		
...
<td><input name="cat_name" type="text" id="cat_name" value="<?php echo $editingrecord->cat_name; ?>" size="40" />
