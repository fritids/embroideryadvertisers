...
{
	$output = "";
	$output .= '<div class="eStore_fancy1_cart_wrapper">';
	if (!digi_cart_not_empty()){
		$output .= eStore_empty_cart_display();
...
{
	$output = "";
	$output .= '<div class="eStore_fancy2_cart_wrapper">';
	if (!digi_cart_not_empty()){
		$output .= eStore_empty_cart_display();
		$output .= "</div>";//end wrapper
        return $output;
	...
	if (get_option('eStore_display_continue_shopping')){		
    	$products_page = get_option('eStore_products_page_url');
    	$output .= '<tr><td colspan="4"><a href="'.$products_page.'" class="eStore_cart_fancy_2_continue_shopping">'.ESTORE_CONTINUE_SHOPPING.'</a></td></tr>';             
	}
...
{
	$output = "";
	$output .= '<div class="eStore_classic_cart_wrapper">';
	$title = get_option('wp_cart_title');
	if (!digi_cart_not_empty()){
...
function eStore_cart_display_shipping_cost_part($currency_symbol,$decimal,$postage_cost,$shipping_price_class="eStore_cart_shipping_price")
{
    ...
    if($postage_cost != 0){
		$output .= "<tr><td colspan='2' style='font-weight: bold; text-align: right;'>".ESTORE_SHIPPING.": </td><td style='text-align: left'><span class='".$shipping_price_class."'>".print_digi_cart_payment_currency($postage_cost, $currency_symbol, $decimal)."</span></td><td></td></tr>";
    }
    return $output;
}
...
function eStore_cart_display_total_part_fancy2($currency_symbol,$decimal,$total,$postage_cost,$total_tax=0)
{
	$output = "";
	if($postage_cost != 0 || $total_tax != 0){
		$output .= "<tr><td colspan='2' style='font-weight: bold; text-align: right;'>".ESTORE_SUB_TOTAL.": </td><td style='text-align: left'><span class='eStore_cart_fancy_2_price'>".print_digi_cart_payment_currency($total, $currency_symbol, $decimal)."</span></td><td></td></tr>";
	}
	
	$output .= eStore_cart_display_shipping_cost_part($currency_symbol,$decimal,$postage_cost,"eStore_cart_fancy_2_price");

    if($total_tax != 0){
    	$output .= "<tr><td colspan='2' style='font-weight: bold; text-align: right;'>".WP_ESTORE_TAX.": </td><td style='text-align: left'><span class='eStore_cart_fancy_2_price'>".print_digi_cart_payment_currency($total_tax, $currency_symbol, $decimal)."</span></td><td></td></tr>";
    }
    $output .= "<tr><td colspan='2' style='font-weight: bold; text-align: right;'>".ESTORE_TOTAL.": </td><td style='text-align: left'><span class='eStore_cart_fancy_2_price'>".print_digi_cart_payment_currency(($total+$postage_cost+$total_tax), $currency_symbol, $decimal)."</span></td>";
    $output .= "<td></td></tr>";	
    return $output;
}
...
function eStore_cart_display_checkout_button_form_part()
{
	...
	//Create the checkout button form
            ...
            {
                //eStore_load_checkout_method_jquery(); - in the footer
