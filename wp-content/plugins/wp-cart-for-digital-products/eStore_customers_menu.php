...
function wp_estore_add_customer_menu()
{
	...
	if (isset($_POST['Submit']))
	{
		...
		$post_editedcustomer = $wpdb->escape($_POST['editedcustomer']);
		$post_firstname = $wpdb->escape($_POST['firstname']);
		$post_lastname = $wpdb->escape($_POST['lastname']);
		$post_emailaddress = $wpdb->escape($_POST['emailaddress']);
		$post_purchasedproductid = $wpdb->escape($_POST['purchasedproductid']);
		if($post_purchasedproductid=='')$post_purchasedproductid=0;
		
		$post_txn_id = $wpdb->escape($_POST['txn_id']);
		$post_date = $wpdb->escape($_POST['date']);
		if(empty($post_date)){
			$post_date = (date ("Y-m-d"));
		}
		$clienttime	= (date ("H:i:s"));
		$post_sale_amt = $wpdb->escape($_POST['sale_amount']);
		$post_coupon_code_used = $wpdb->escape($_POST['coupon_code_used']);
		$post_member_username = $wpdb->escape($_POST['member_username']);
			
		$post_product_name = $wpdb->escape(stripslashes($_POST['product_name']));
		$tmpaddress = htmlentities(stripslashes($_POST['address']) , ENT_COMPAT);
		$post_address = $wpdb->escape($tmpaddress);
		$post_phone = $wpdb->escape($_POST['phone']);
		$post_qty_purchased = $wpdb->escape($_POST['purchase_qty']);	
		$post_subsc_id = "";	
		$post_ipaddress = $wpdb->escape($_POST['ipaddress']);		
		$post_status = $wpdb->escape($_POST['status']);
		$post_sr_number = $wpdb->escape($_POST['serial_number']);
		$post_notes = $wpdb->escape($_POST['notes']);
	...
	{
        $post_orig_customer_id = $wpdb->escape($_POST['orig_customer_id']);
...
function display_customer_list($ret_customer_db)
{
	...
	<th scope="col">'.__("Purchased Product's ID", 'wp_eStore').'</th>
	...
	if ($ret_customer_db)
	{
		foreach ($ret_customer_db as $ret_customer_db)
		{
			...
			$output .= '<td><strong>'.$ret_customer_db->first_name.'</strong></td>';
			$output .= '<td><strong>'.$ret_customer_db->last_name.'</strong></td>';
			$output .= '<td><strong>'.$ret_customer_db->email_address.'</strong></td>';
			$output .= '<td><strong>'.$ret_customer_db->purchased_product_id.'</strong></td>';
			$output .= '<td><strong>'.$ret_customer_db->txn_id.'</strong></td>';
			$output .= '<td><strong>'.$ret_customer_db->date.'</strong></td>';
			$output .= '<td><strong>'.$ret_customer_db->sale_amount.'</strong></td>';
			if(!empty($ret_customer_db->coupon_code_used))
			{
				$output .= '<td><strong>'.$ret_customer_db->coupon_code_used.'</strong></td>';
			}
			else
			{
				$output .= '<td><strong>--</strong></td>';
			}
			$output .= '<td><strong>'.$ret_customer_db->status.'</strong></td>';			
...
function eStore_display_customer_list_limit($limit)
{
	...
	if ($wp_eStore_db)
	{
		foreach ($wp_eStore_db as $wp_eStore_db)
		{
			...
			$output .= '<td><strong>'.$wp_eStore_db->first_name.'</strong></td>';
			$output .= '<td><strong>'.$wp_eStore_db->last_name.'</strong></td>';
			$output .= '<td><strong>'.$wp_eStore_db->email_address.'</strong></td>';
			$output .= '<td><strong>'.$wp_eStore_db->purchased_product_id.'</strong></td>';
			$output .= '<td><strong>'.$wp_eStore_db->txn_id.'</strong></td>';
			$output .= '<td><strong>'.$wp_eStore_db->date.'</strong></td>';
			$output .= '<td><strong>'.$wp_eStore_db->sale_amount.'</strong></td>';
			if(!empty($wp_eStore_db->coupon_code_used))
			{
				$output .= '<td><strong>'.$wp_eStore_db->coupon_code_used.'</strong></td>';
			}
			else
			{
				$output .= '<td><strong>--</strong></td>';
			}
			$output .= '<td><strong>'.$wp_eStore_db->status.'</strong></td>';	
