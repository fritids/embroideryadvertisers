...
function wp_eStore_write_debug_array($array_to_write,$success,$end=false,$debug_log_file_name='')
{
	...
	if(empty($debug_log_file_name)){
    	$debug_log_file_name = realpath(dirname(__FILE__)).'/eStore_post_payment_debug.log';//TODO - replace with WP_ESTORE_PATH
	}else{
		$debug_log_file_name = realpath(dirname(__FILE__)).'/'.$debug_log_file_name;//TODO - replace with WP_ESTORE_PATH
	}	
    // Write to log
    $fp=fopen($debug_log_file_name,'a');
    fwrite($fp, $text );
