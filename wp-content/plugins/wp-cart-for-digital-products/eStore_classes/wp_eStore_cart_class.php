...
class WP_eStore_Cart
{
    ...
    {
    	$prod_name = $prod_data_array['product'];
    ...
    {    	
		...
		stripslashes($prod_data_array['product']),
		...
		if($prod_data_array['digital_flag']=="1"){
...
function wp_eStore_save_cart_details_to_db($eStore_cart)
{
	...
	$serialized_cart = $wpdb->escape(serialize($eStore_cart));
...
{	
	...
	foreach ($items_object_array as $eStore_item)
	{
		...
		$product = array('name' => $eStore_item->item_name, 
