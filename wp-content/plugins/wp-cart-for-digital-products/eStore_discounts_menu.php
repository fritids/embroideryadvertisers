...
function wp_estore_discounts_menu()
{
	...
	{	
		...
		if(empty($error_message))
		{
			...
			$post_coupon_code = $wpdb->escape($_POST['coupon_code']);
			$post_discount_value = $wpdb->escape($_POST['discount_value']);
			$post_discount_type = $wpdb->escape($_POST['discount_type']);
			$redemption_limit = $wpdb->escape($_POST['redemption_limit']);
			if(empty($redemption_limit)){
				if($redemption_limit != '0'){
					$redemption_limit = '9999';
				}
			}
			$redemption_count = $wpdb->escape($_POST['redemption_count']);
	        $post_coupon_active = $wpdb->escape($_POST['coupon_active']);
	        $post_property = $wpdb->escape($_POST['property']);
	        $post_logic = $wpdb->escape($_POST['logic']);
	        $post_value = $wpdb->escape($_POST['value']);
	        
			$start_date = $wpdb->escape($_POST['start_date']);
			if(empty($start_date)){$start_date="0000-00-00";}

	        $expiry_date = $wpdb->escape($_POST['expiry_date']);
    ...
    <div class="submit">
        <input type="submit" name="auto_discount_settings" value="<?php _e('Update'); ?> &raquo;" />
    </div>
    </form>
	</div></div>
...
?>
    ...
    <div class="submit">
        <input type="submit" name="marketing_settings" value="<?php _e('Update'); ?> &raquo;" />
    </div>
    </form>
    </div></div>
<?php
