...
function wp_estore_product_management_menu()
{
    ...
    <div class="submit">
        <input type="submit" name="info_update" value="Search &raquo;" />
    </div>   
    </form>
    </div></div>
	<?php
...
function eStore_display_products($limit)
{
    ...
    .column-product_actions {width:6%;}
    </style>
...
function wp_estore_add_product_menu()
{
	...
	if (isset($_POST['Submit']))
	{
		...
		$post_producturl = trim($wpdb->escape($_POST['producturl']));
		...
		$post_product_button_image_url = trim($wpdb->escape($_POST['buttonimageurl']));
		$post_product_return_url = trim($wpdb->escape($_POST['returnurl']));
		...
		$post_product_thumbnail = trim($wpdb->escape($_POST['thumbnail_url']));
		...
		$post_product_commission = trim($wpdb->escape($_POST['productcommission']));
        ...
        $aweber_list = $wpdb->escape(trim(stripslashes($_POST['aweber_list'])));
        $currency_code = $wpdb->escape($_POST['currency_code']);
        $target_thumb_url = trim($wpdb->escape($_POST['target_thumb_url']));
        $target_button_url = trim($wpdb->escape($_POST['target_button_url']));
        $weight = $wpdb->escape($_POST['itemweight']);
        $product_url = trim($wpdb->escape($_POST['product_url']));
        ...
        $post_author_id = trim($wpdb->escape($_POST['author_id']));   
        ...
        $post_rev_share_commission = $wpdb->escape($_POST['rev_share_commission']);
        ...
        $post_per_customer_qty_limit = $wpdb->escape($_POST['per_customer_qty_limit']);
		...
		if ($post_editedproduct=='')//Insert the new product
		{
			...
			$available_key_codes = $wpdb->escape($_POST['available_key_codes']);			
	...
	{
		$theproduct = $wpdb->escape($_POST['prod_id']);
...
<?php wp_eStore_show_file_upload_more_info(); ?>
<p class="description">This is useful when you want to customize the look of your buy button using a custom button image for this product.</p></td>
</tr>
...
<p class="description">If checked the digital content will be offered as an anonymous download (the customer won\'t know where it is coming from). If Unchecked the buyer will be redirected to the above URL when they click on the encrypted link (useful for video on demand system)</p></td>
</tr>

<tr valign="top">
<th scope="row">Pay Per View Content</th>
<td><input type="checkbox" name="ppv_content" <?php if ($editingproduct->ppv_content=='1'){echo 'checked="checked"';} ?> />
<br /><p class="description">If you are offering Pay Per View content then check this box and uncheck the "Downloadable" checkbox above. For pay per view content, the true URL of the page where the content is embedded (e.g. a steaming video) does not get revealed.</p></td>
</tr>
...
<div class="msg_body">
<p style="color:red">
<strong>The options in this section are only used for "Buy Now" or "Subscription" type buttons. <a href="http://www.tipsandtricks-hq.com/forum/topic/different-types-of-payment-buttons-and-their-behaviour" target="_blank">Explanation on the different types of payment buttons</a></strong>
</p>
...
?>
<p class="submit"><input type="submit" name="Submit" value="Save Product" /></p>
...
<input type="submit" name="copy_product" value="Copy Product Details" />
</form>
</div>
</div>
...
<?php
	...
	echo '<a href="admin.php?page=wp-cart-for-digital-products/wp_eStore1.php" class="button rbutton">Manage Products</a>&nbsp;&nbsp;';
	echo '<a href="admin.php?page=wp_eStore_addedit" class="button rbutton">Create New Product</a><br /><br />';
