...
function is_quantity_availabe($id,$requested_quantity,$prod_name="")
{
    ...
    {    	
	    ...
	    if (isset($_REQUEST['eStore_cquantity'])){//This is a change quantity request in the cart
	    	...
	    	$total_qty_for_this_item = $current_item_qty_in_cart + $purchased_qty + $requested_quantity;
	    }
...
function print_digi_cart_payment_currency($price, $symbol, $decimal='.', $thousands_sep=',')
{
	...
	if(is_numeric($price)){//format the price amount
		if(WP_ESTORE_SHOW_CURRENCY_SYMBOL_AFTER_AMOUNT==='1'){
			return number_format($price, 2, $decimal, $thousands_sep) . $symbol;
		}else{
...
function wp_eStore_load_libraries()
{
    ...
    if(WP_ESTORE_ENABLE_AJAX_ON_ADD_TO_CART_BUTTONS === '1'){//add ajax handling scripts
	    wp_enqueue_script('estore-plugins-js', WP_ESTORE_LIB_URL.'/eStore_plugins.js');//TODO - minify this file
	    wp_enqueue_script('estore-ajax-js', WP_ESTORE_LIB_URL.'/eStore-ajax.js');
...
{
	if(isset($_REQUEST['eStore_checkout']) && $_REQUEST['eStore_checkout']=="process"){//Process checkout
