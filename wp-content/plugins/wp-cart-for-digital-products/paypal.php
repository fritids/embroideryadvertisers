...
class paypal_ipn_handler {
   ...
   var $sandbox_mode = false;
   	...
   	function validate_and_dispatch_product()
	{
        ...
        $total_shipping = number_format(($this->ipn_data['mc_handling'] + $this->ipn_data['mc_shipping']),2);
        $total_minus_total_tax = number_format(($total_purchase_amt - $total_tax),2);
        ...
        if (!$recurring_payment)
        {
    		...
    		foreach ($cart_items as $current_cart_item)
    		{
    			...
    			$this->debug_log('Current available copies value: '.$retrieved_product->available_copies.' Sales count value: '.$retrieved_product->sales_count,true);
