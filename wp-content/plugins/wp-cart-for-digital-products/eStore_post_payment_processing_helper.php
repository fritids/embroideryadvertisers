...
function eStore_apply_post_payment_dynamic_tags($string_to_process,$payment_data,$cart_items='',$additional_data='')
{
    ...
    if(!empty($additional_data)){//($payment_data['gateway'] != "manual")
...
function eStore_stamp_pdf_file($payment_data,$src_file)
{	
...
function eStore_handle_refund($payment_data)
{
    ...
    if (function_exists('wp_aff_platform_install'))
    {
    	...
    	{
			// Check for the original transaction in the database
			$resultset = $wpdb->get_results("SELECT * FROM $customer_table_name WHERE txn_id = '$parent_txn_id'", OBJECT);
			if($resultset)
			{
				eStore_payment_debug('Found the original transation in the database. Reverse the commmission for this payment. Parent Txn ID: '.$parent_txn_id,true);
				wp_aff_handle_refund($parent_txn_id);						
			}    	
			else{
				eStore_payment_debug('Could not find the original transation in the database. Nothing to reverse.',true);
			}    		   		
    	}        
    }
...
function is_paypal_recurring_payment($payment_data)
{
    ...
    {        
        ...
        $customer_exists = $wpdb->get_row("SELECT * FROM $customer_table_name WHERE email_address = '$email' and purchased_product_id = '$item_number'", OBJECT);
        if($customer_exists)
        {
...
function eStore_POST_IPN_data_to_url($data,$post_url='',$cart_items='')
{
	...
	if(!empty($ipn_external_post_url))
	{
		...
		if($retVal == "NO CURL"){
			...
			eStore_payment_debug('IPN values posted successfully. Return value: '.$retVal,false);
		}
	}	
}
...
function eStore_register_link_in_db($creation_time='',$download_key='',$download_item='',$download_limit_count,$download_limit_time,$download_limit_ip='',$access_count=0,$txn_id='')
{
    ...
    $fields['download_item'] =  $wpdb->escape($download_item);
...
function record_sales_data($payment_data,$cart_items)
{
    ...
    $clientdate = (date ("Y-m-d"));
    $clienttime	= (date ("H:i:s"));		    
    $address = $wpdb->escape(stripslashes($payment_data['address']));
    $phone = $payment_data['phone'];
    $coupon_code_used = $wpdb->escape($payment_data['coupon_used']);
    $eMember_id = $wpdb->escape($payment_data['eMember_userid']);
	...
	{		
            ...
            eStore_payment_debug('Current available copies value: '.$retrieved_product->available_copies.' Sales count value: '.$retrieved_product->sales_count,true);
...
function eStore_aff_award_commission($payment_data,$cart_items,$customReferrer='')
{
	...
	if (eStore_affiliate_capability_exists())
	{
		...
		if(WP_ESTORE_CHECK_EMEMBER_REFERRER_FOR_AFFILIATE_ID === '1' && !empty($eMember_member_id)){
			...
			$eMember_resultset = dbAccess::find(WP_EMEMBER_MEMBERS_TABLE_NAME, ' member_id=' . $wpdb->escape($eMember_member_id));
		...
		if (!empty($referrer))
		{
            ...
            $second_tier_commission_amount = number_format($second_tier_commission_amount,2);
			...
			if(get_option('eStore_aff_enable_commission_per_transaction')!='')
			{
				...
				if (get_option('wp_aff_use_fixed_commission'))
				{
					eStore_payment_debug('Using fixed commission model.',true);				
					if($aff_version>=4.7){
		            	eStore_payment_debug("Awarding fixed affiliate commission - Affiliate version is greater than 4.7",true);		            	
					...
					//For percentage based commission there is not difference between per transaction commission amount and the per item commission amount
				}				
			}
		    
			if($aff_version>=4.7)
            {
            	eStore_payment_debug("Affiliate version is greater than 4.7",true);
            	$updatedb = "INSERT INTO $aff_sales_table (refid,date,time,browser,ipaddress,payment,sale_amount,txn_id,item_id,buyer_email,campaign_id,buyer_name) VALUES ('$referrer','$clientdate','$clienttime','','$clientip','$commission_amount','$sale_amount','$txn_id','$item_id','$buyer_email','$c_id','$buyer_name')";
            }			            
            else if($aff_version>=4.4)
            {
            	eStore_payment_debug("Affiliate version is greater than 4.4",true);
            	$updatedb = "INSERT INTO $aff_sales_table (refid,date,time,browser,ipaddress,payment,sale_amount,txn_id,item_id,buyer_email,campaign_id) VALUES ('$referrer','$clientdate','$clienttime','','$clientip','$commission_amount','$sale_amount','$txn_id','$item_id','$buyer_email','$c_id')";
            }
            else if($aff_version>=1.1)
            {
            	eStore_payment_debug("Affiliate version is greater than 1.1",true);
            	$updatedb = "INSERT INTO $aff_sales_table (refid,date,time,browser,ipaddress,payment,sale_amount,txn_id,item_id,buyer_email) VALUES ('$referrer','$clientdate','$clienttime','','$clientip','$commission_amount','$sale_amount','$txn_id','$item_id','$buyer_email')";
              	//$updatedb = "INSERT INTO $aff_sales_table VALUES ('$referrer','$clientdate','$clienttime','','','$commission_amount','$sale_amount','$txn_id','$item_id','$buyer_email')";
            }
            else
            {
            	$updatedb = "INSERT INTO $aff_sales_table (refid,date,time,browser,ipaddress,payment,sale_amount) VALUES ('$referrer','$clientdate','$clienttime','','$clientip','$commission_amount','$sale_amount')";
			...
			if(function_exists('wp_aff_send_commission_notification')){
				if($commission_amount>0)
				{
					...
					eStore_payment_debug("Sent commission email notification request to the affiliate plugin",true);
				}
				else
				{
