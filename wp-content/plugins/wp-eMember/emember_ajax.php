...
function emember_ajax_login(){ 
    ...
    $url = $membership_level_resultset->loginredirect_page;
...
function wp_emem_process_delete_image($id){ 
	...
	if(!empty($old_file[0]))@unlink($upload_path.$old_file[0]);
   ...
   if(isset($_GET['mem'])&&($_GET['mem']!=-1)){
    ...
    if(count($wp_users)>0):            
        ...
        <tr valign="top" <?php echo ($count%2)?"class='alternate'":"";?>>
            ...
            <td><?php $expires = emember_get_expiry_by_member_id($wp_user->member_id);
                    if($expires == 'noexpire') echo "Until Cancelled"; else echo date(get_option('date_format'),strtotime($expires));?><!--<input type="text" size="12" readonly="readonly" value="<?php echo $wp_user->last_accessed_from_ip;?>" onfocus="this.select();" onclick="this.select();">--></td>
...
function item_list_ajax(){
    ...
    if(emember_is_ajax()){
		...
		$protected_post     = (array)unserialize($level->post_list);
		$protected_page     = (array)unserialize($level->page_list);
		$protected_comment  = (array)unserialize($level->comment_list);
		$protected_category = (array)unserialize($level->category_list);
		$protected_attachment = (array)unserialize($level->attachment_list);
		$protected_custom_post = (array)unserialize($level->custom_post_list); 
		$disable_bookmark   = (array)unserialize($level->disable_bookmark_list);
        switch($tab){
            case 'pages':    
				$bookmarks = isset($disable_bookmark['pages'])?$disable_bookmark['pages']:array();
                $args = array(
                    'child_of' => 0,
                    'sort_order' => 'ASC',
...
//                $all_pages      =  $wpdb->last_result;               
                ...
                foreach($all_pages as $page){
					...
					$page_summary['protected']  = in_array($page->ID,$protected_page)? "checked='checked'": "";              
					$page_summary['bookmark']   = in_array($page->ID,$bookmarks)? "checked='checked'": ""; 
                ...
                break;
            case 'posts':
				$bookmarks = isset($disable_bookmark['posts'])?$disable_bookmark['posts']:array();
...
//                                                  'post_type'=>'any'));
                ...
                foreach($all_posts as $post){
                    ...
                    $post_summary['protected']  = in_array($post->ID,$protected_post)? "checked='checked'": ""; 
					$post_summary['bookmark']   = in_array($post->ID,$bookmarks)? "checked='checked'": ""; 
            ...
            case 'comments':
                ...
                foreach($all_comments as $comment){
					  ...
					  $comment_summary['protected']  = in_array($comment->comment_ID,$protected_comment)? "checked='checked'": ""; 
            ...
            case 'categories':
                ...
                foreach($all_categories as $category){
					  ...
					  $category_summary['protected']  = in_array($category->term_id,$protected_category)? "checked='checked'": ""; 
			...
			case 'attachments':
                ...
                foreach($all_posts as $post){
                    $post_summary = array();
                    $user_info    = get_userdata($post->post_author);
                    $post_summary['protected']  = in_array($post->ID,$protected_attachment)? "checked='checked'": ""; 
                    $post_summary['ID']         = $post->ID;
                    $post_summary['date']       = $post->post_date;
                    $post_summary['title']      = isset($post->post_title)?$post->post_title:"";
                    $post_summary['author']     = isset($user_info->user_nicename)?$user_info->user_nicename:"";
			...
			case 'custom-posts':
				...
				if(!empty($arg)){
					...
					foreach($all_posts as $post){
						$post_summary = array();
						$user_info    = get_userdata($post->post_author);
						$post_summary['protected']  = in_array($post->ID,$protected_custom_post)? "checked='checked'": ""; 
						$post_summary['ID']         = $post->ID;
						$post_summary['date']       = $post->post_date;
						$post_summary['title']      = isset($post->post_title)?$post->post_title:"";
						$post_summary['author']     = isset($user_info->user_nicename)?$user_info->user_nicename:"";
...
function wp_emem_check_level_name(){
    if(emember_is_ajax()){        
        ...
        if($user) echo '[ "' . $_GET['fieldId'] . '",false, "&chi;&nbsp;'.EMEMBER_ALREADY_TAKEN.'"]';
        else echo '[ "' . $_GET['fieldId'] . '",true, "&radic;&nbsp;'.EMEMBER_STILL_AVAIL.'"]';
    }
    exit(0);
}
...
function emember_load_membership_form(){
    ...
    if(empty($id)){
        ...
        $level = dbAccess::find(WP_EMEMBER_MEMBERSHIP_LEVEL_TABLE," id = '" . $wpdb->escape($id) . " ' ");
        $role = $level->role;
        $name = $level->alias;
