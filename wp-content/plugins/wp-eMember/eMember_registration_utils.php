...
function emember_process_free_rego_with_confirm_form(){
			...
			if($enable_recaptcha){
		    	if(isset($_POST["recaptcha_response_field"])){
		            ...
		            if(!$resp->is_valid){
                        ...
                        $emember_config->set_stacked_message('emember_free_registration_confirm', 
                                             array('type'=>'error','message'=>$error_message));
		            }            
		        }
		        else{
		        	$output .= '<span class="emember_error">reCAPTCHA&trade; service encountered error. please Contact Admin. </span>';                            
                    $emember_config->set_stacked_message('emember_free_registration_confirm', 
                                         array('type'=>'warning','message'=>$output));
                }
		    }						
	...
	//				$ret = dbAccess::insert(WP_EMEMBER_MEMBERS_TABLE_NAME, $fields);
				    ...
				    eMember_log_debug("Sending registration completion email for free registration with confirmation to: ".$_POST['wp_emember_aemail'],true); 
                    ...
                    $emember_config->set_stacked_message('emember_free_registration_confirm', 
                                         array('type'=>'success','message'=>$output));
				}
		    }
		}
		else{
...
function free_rego_with_email_confirmation_handler($args=NULL){
    if (!is_null($args)){ extract(shortcode_atts(array('level' => ''), $args));}
    else{$level = '';}
	global $wpdb;
    $emember_config = Emember_Config::getInstance();    
	$error_message = "";
...
<?php 
	if(!empty($level)){echo "<input type=\"hidden\" name=\"wp_emember_membership_level\" value=\"$level\">";}   
?> 
...
function emember_process_reg_form(){
    ...
    if(is_blocked_ip(get_real_ip_addr())){
        ...
        $emember_config->set_stacked_message('emember_full_registration', 
	...
	if(isset($_POST['eMember_Register']))
	{
		...
		if($enable_recaptcha){
		    ...
		    if(isset($_POST["recaptcha_response_field"])){
		        ...
		        if(!$resp->is_valid){
                    ...
                    $emember_config->set_stacked_message('emember_full_registration',   
                                         array('type'=>'error','message'=>$output));
		        }            
		    }
		    else{
		        $output .= '<span class="emember_error">reCAPTCHA&trade; service encountered error. please Contact Admin. </span>';                            
		...
		if(!$enable_recaptcha||($resp && $resp->is_valid)){
		    ...
		    {  
		    	...
		    	if(is_array($_POST['emember_custom'])){
		        ...
		        if($is_reg_successfull){  
	            	...
	            	{	            		
	            		if (!empty($redirect_page)){
							...
							$redirect_page = $redirect_page.$separator."doLogin=1&emember_u_name=".$_POST['wp_emember_user_name']."&emember_pwd=".$_POST['wp_emember_pwd'];
...
?>
    ...
    <tr>
       ...
       <td><input type="text" id="wp_emember_user_name" name="wp_emember_user_name" size="20" value="<?php echo strip_tags(isset($_POST['wp_emember_user_name'])?$_POST['wp_emember_user_name']:"");?>" class="validate[required,custom[onlyLetterNumberUnderscore],minSize[4]<?php echo $letter_number_underscore?>,ajax[ajaxUserCall]] eMember_text_input" /></td>
    </tr>
    <tr>
    ...
    <tr>
       ...
       <td>
       	   <select name="wp_emember_title">
               <option value="Mr"><?php echo EMEMBER_MR;?></option>
               <option value="Mrs"><?php echo EMEMBER_MRS;?></option>
               <option value="Miss"><?php echo EMEMBER_MISS;?></option>
               <option value="Ms"><?php echo EMEMBER_MS;?></option>
               <option value="Dr"><?php echo EMEMBER_DR;?></option>
           </select>
       </td>
     </tr>
     <?php endif;?>
    ...
    <tr>
       ...
       <td><input type="text" name="wp_emember_country" id="wp_emember_country" size="20" value="<?php echo strip_tags(isset($_POST['wp_emember_country'])?$_POST['wp_emember_country']:""); ?>" class="<?php echo $emember_config->getValue('eMember_reg_country_required')? 'validate[required] ': "";?>eMember_text_input" /></td>
    </tr>
    <?php endif;?>
    ...
    echo apply_filters('emember_captcha', ""); ?>  </td></tr>
