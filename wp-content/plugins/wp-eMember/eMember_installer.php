	...
	if($wpdb->get_var("SHOW TABLES LIKE '" . $wpememmeta->get_table('membership_level') . "'") != $wpememmeta->get_table('membership_level')){
	   ...
	   if(is_null($results)){
	      $sql = "INSERT INTO  ".$wpememmeta->get_table('membership_level')."  (
	            ...
	            options,
	            campaign_name
	         )VALUES (NULL , 'Content Protection', 'administrator', '15', '0',NULL,NULL, NULL,NULL,NULL , NULL , NULL , NULL,NULL,NULL,''
	         );";
	      $wpdb->query($sql);
	   }	   
	...
	if( $installed_ver != $wpememmeta->get_db_version() )
	{
	   ...
	   if(is_null($results)){
	      $sql = "INSERT INTO  ".$wpememmeta->get_table('membership_level')."  (
	            ...
	            disable_bookmark_list,
	            options
	         )VALUES (NULL , 'Content Protection', '1', '0', '0', NULL,NULL,NULL,NULL, NULL , NULL , NULL,NULL,NULL
	         );";
	      $wpdb->query($sql);
	   }
       $sql = "CREATE TABLE " .$wpememmeta->get_table('uploads') . " (
...
function wp_emember_migrate_protection_list(){//TODO - remove @v10
	...
	foreach($results as $result){
		...
		if(!empty($attachments))  $permissions += 16;
		if(!empty($custom_posts)) $permissions += 32;
