...
function emember_wp_username_exists($username){
    if(!eMember_is_multisite_install()) return username_exists($username);
...
function emember_wp_email_exists($email){
    if(!eMember_is_multisite_install()) return email_exists($email);
...
function update_wp_user_Role($wp_user_id, $role){
	...
	if(is_array($caps) && in_array('administrator', array_keys((array)$caps))) return;
...
function emember_get_after_login_page_url_of_current_user()
{
	...
	if(!empty($after_login_page)){return $after_login_page;}//This member has a member specific welcome page
	
	$membership_level_resultset = $emember_auth->userInfo->primary_membership_level;  
	$after_login_page = $membership_level_resultset->loginredirect_page;       	
	if(!empty($after_login_page)){return $after_login_page;}//This member has a membership level specific welcome page
	
	$after_login_page = $emember_config->getValue('after_login_page'); 
	if(!empty($after_login_page)){return $after_login_page;}//Global member welcome page
...
function emember_date_locale($timestamp){
    ...
    if($lang != 'eng') return date('d-m-Y', $timestamp);  
...
function emember_replace_avatar( $avatar,$id_or_email='', $size = 96, $default= null, $alt='' ){
    ...
    if(empty($id_or_email)) return $avatar; //not a user. so dont modify it.
    if(is_numeric($id_or_email)){
        $user_details = get_userdata( $id_or_email );
        if($user_details)$emember_email = $user_details->user_email;
    }
        ...
        if(!empty($id_or_email->user_id)){
            ...
            if($user)$emember_email = $user->user_email;
...
function emember_update_membership_level($member_id, $target_membership_level){
    ...
    $target_level_info = dbAccess::find(WP_EMEMBER_MEMBERSHIP_LEVEL_TABLE,' id=' . $wpdb->escape($target_membership_level));    
...
function get_wp_emember_custom_field_val($args)
{
	...
	if (isset($_SESSION['ap_id'])){$referrer = $_SESSION['ap_id'];}
