...
function auth_check_attachment($content){ 
	...
	if($emember_auth->is_protected_attachment($id)){
		...
		if(isset($emember_auth->hasmore[$id])){ unset($emember_auth->hasmore );}
		$emember_auth->is_post_visible = true;
		return $content;
	}   
    //}
        ...
        if($emember_auth->is_protected_custom_post($id)){
            if($emember_auth->isLoggedIn()){
                ...
                if(isset($_GET['event'])&&($_GET['event']=='login'))                   
                    ...
                    if(isset($emember_auth->hasmore[$id])){unset($emember_auth->hasmore);return $content;}
                    return $before_more . get_login_link();                        
                }                    
            }
        }
...
function check_page_content($id,$content){
   global $emember_auth; 
   $emember_auth = Emember_Auth::getInstance();
   if($emember_auth->is_my_page_post($id)) return $content;
   $emember_config = Emember_Config::getInstance();    
    global $post;
//    $id = $post->ID;
    global $more;
        ...
        if($emember_auth->is_protected_comment($id)){
	       if($emember_auth->isLoggedIn()){
	           ...
	           if(isset($_GET['event'])&&($_GET['event']=='login'))                   
	               ...
	               if(isset($emember_auth->hasmore[$id])){unset($emember_auth->hasmore[$id]);return $content;}
	               return get_login_link();                                
	       }    	
	    }
	    else{
	       if(isset($emember_auth->hasmore[$id])){ unset($emember_auth->hasmore[$id]);}        
	       return $content;
	    }
    }  //
    else{
...
function emember_protected_handler($attrs, $contents, $codes=''){
    ...
    if(emember_is_first_click()) return $contents; //google first click free enabled.
    ...
    if(!$emember_auth->isLoggedIn()){ 
    	...
    	if(isset($attrs['scope'])&&($attrs['scope']=="not_logged_in_users_only")) return $contents;
    }

    if($emember_auth->isLoggedIn()) 
    {
    	// Do not show the content to anyone who is logged in
    	if(isset($attrs['scope'])&&($attrs['scope']=="not_logged_in_users_only")) return "";
    	
    	// Show content to anyone who is logged in
    	if(isset($attrs['scope'])&&($attrs['scope']=="verified_users_only")) return $contents;
