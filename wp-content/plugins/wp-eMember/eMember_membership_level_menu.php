...
function wp_eMember_membership_level()
{
    ...
    $_GET['level_action'] = isset($_GET['level_action'])?$_GET['level_action']:"";
...
function content_protection(){
    if(isset($_POST['submit'])){	  
		...
		switch($_POST['content_type']){
			case 'posts':  
				$old_ids = unserialize( $level->post_list );
				...
				break;
			case 'pages':
				$old_ids = unserialize( $level->page_list );
				$ids = emember_adjust_list($ids, $page_ids, $old_ids);
				$ids = serialize($ids);
				...
				break;
			case 'comments':
				$old_ids = unserialize( $level->comment_list );
				$ids = emember_adjust_list($ids, $page_ids, $old_ids);
				$ids = serialize($ids);
				$fields = array('comment_list'=>$ids);
				break;
			case 'categories':
				$old_ids = unserialize( $level->category_list );
				$ids = emember_adjust_list($ids, $page_ids, $old_ids);
				$ids = serialize($ids);
				$fields = array('category_list'=>$ids);
				break;
		    case 'attachments':
				$old_ids = unserialize( $level->attachment_list );
				$ids = emember_adjust_list($ids, $page_ids, $old_ids);
				$ids = serialize($ids);
				$fields = array('attachment_list'=>$ids);			
				break;
			case 'custom-posts':
				$old_ids = unserialize( $level->custom_post_list );
				$ids = emember_adjust_list($ids, $page_ids, $old_ids);
				$ids = serialize($ids);
...
function manage_access_levels(){
   ...
   if(isset($_POST['add_new'])){
      $alias = $wpdb->escape(stripslashes($_POST['wpm_levels']['new_level']['name']));
      if(empty($alias)){
      ...
      $campaign_name = $wpdb->escape(stripslashes($_POST['wpm_levels']['new_level']['campaign_name']));
      ...
      if($ret===false)
       ...
       foreach($_POST['wpm_levels'] as $id=>$wp_level){
            $alias = $wpdb->escape(stripslashes($wp_level['name']));
            $role  = $wp_level['role']; 
            $login_redirect = $wpdb->escape($wp_level['loginredirect']);
            $campaign_name = $wpdb->escape(stripslashes($wp_level['campaign_name']));
