...
function wp_eMember_admin_functions_menu()
{
	...
	<li <?php echo ($current==1) ? 'class="current"' : ''; ?> ><a href="admin.php?page=eMember_admin_functions_menu">General Admin Functions</a></li>
	<li <?php echo ($current==2) ? 'class="current"' : ''; ?> ><a href="admin.php?page=eMember_admin_functions_menu&tab=2">Download Folder Protection</a></li>
	</ul>
...
function wp_eMember_admin_functions_general_menu()
{
    ...
    if(isset($_POST['emem_to_wp'])){
           ...
           foreach($ret_member_db as $emember){
               ...
               if(empty($emember->user_name)) continue;          	   
           	   if(strtolower($emember->user_name) === "admin") continue;  
