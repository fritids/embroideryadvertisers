...
?>	<h3><?php _e( 'Avatar','simple-local-avatars' ); ?></h3>
	...
	<table class="form-table">
		<tr>
			...
			<td>
			<?php
				if ( ! $upload_rights = current_user_can('read') )
			...
			?>
					...
					<p>
						<?php if ( current_user_can( 'read' ) && did_action( 'wp_enqueue_media' ) ) : ?><a href="#" class="button hide-if-no-js" id="simple-local-avatar-media"><?php _e( 'Choose from Media Library', 'simple-local-avatars' ); ?></a> &nbsp;<?php endif; ?>
	...
	public function ajax_assign_simple_local_avatar_media() {
		...
		if ( empty( $_POST['user_id'] ) || empty( $_POST['media_id'] ) || ! current_user_can( 'read' ) || ! current_user_can( 'edit_user', $_POST['user_id'] ) || empty( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'assign_simple_local_avatar_nonce' ) )
			die;
